...
def train(n_labeled_data):
    ...
    model.compile(SGD(1e-2, 0.9), loss="categorical_crossentropy", metrics=["acc"])

    if not os.path.exists("result_transfer"):
        os.mkdir("result_transfer")

    hist = model.fit(X_train, y_train, batch_size=min(n_labeled_data, 256), 
                     validation_data=(X_test, y_test), epochs=100).history

    with open(f"result_transfer/history_{n_labeled_data:05}.dat", "wb") as fp:
        pickle.dump(hist, fp)
    ...
    plt.savefig(f"result_transfer/embedding_{n_labeled_data:05}.png")
...
if __name__ == "__main__":
    ...
    with zipfile.ZipFile("result_transfer.zip", "w") as zip:
        for f in glob.glob("result_transfer/*"):
            zip.write(f)
