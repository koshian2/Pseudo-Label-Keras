...
class PseudoCallback(Callback):
    ...
    def on_train_end(self, logs):
        ...
        plt.savefig(f"result_pseudo_mobile/embedding_{self.n_labeled_sample:05}.png")
...
def train(n_labeled_data):
    ...
    if not os.path.exists("result_pseudo_mobile"):
        os.mkdir("result_pseudo_mobile")
    ...
    with open(f"result_pseudo_mobile/history_{n_labeled_data:05}.dat", "wb") as fp:
        pickle.dump(hist, fp)
...
if __name__ == "__main__":
    ...
    with zipfile.ZipFile("result_pseudo_mobile.zip", "w") as zip:
        for f in glob.glob("result_pseudo_mobile/*"):
            zip.write(f)
